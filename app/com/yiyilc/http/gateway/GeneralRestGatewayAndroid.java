 ...
 *
 */
public class GeneralRestGatewayAndroid {
	//
	// Source code recreated from a .class file by IntelliJ IDEA
	// (powered by Fernflower decompiler)
	//

	public GeneralRestGatewayAndroid() {
	}

	public static int handle(String key, int allowTimespanSeconds, HttpServletRequest request,
			HttpServletResponse response, GeneralRestGatewayInterface grgi, StringBuilder errorDescription)
			throws IOException {
			...
			if (keys.isEmpty()) {
				...
				if (!StringUtils.isBlank(_s) && !StringUtils.isBlank(_t)) {
					...
					for (int i = 0; i < parameters.size(); ++i) {
						signData.append(
								(String) parameterNames.get(i) + "=" + (String) parameters.get(parameterNames.get(i))
										+ (i < parameters.size() - 1 ? "&" : ""));
					}

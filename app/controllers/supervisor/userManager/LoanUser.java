...
public class LoanUser extends SupervisorController {
	...
	public static void detail(String sign){
		...
		if(Cache.get(Constants.DICT_KEYS_PROVINCES) == null) {
			provinces = t_dict_ad_provinces.findAll();
			Cache.set(Constants.DICT_KEYS_PROVINCES, provinces);
		} else {
			provinces = (List<t_dict_ad_provinces>)Cache.get(Constants.DICT_KEYS_PROVINCES);
		}
		List<t_dict_ad_citys> citys = User.queryCity(user.provinceId);

		render(user,provinces,citys);
	}

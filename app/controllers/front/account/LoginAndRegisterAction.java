...
public class LoginAndRegisterAction extends BaseController {
	...
	public static void logout() {
		...
		int financeType = user.financeType;
		user.logout(error);
		...
		if(financeType == FinanceTypeEnum.BORROW.getCode()) {
			loginLoan();
		} else {
			login();
		}
	...
	public static void logining() {
		...
		String financeType=params.get("financeType");//新增参数   借款端传值0
		if(StringUtils.isBlank(financeType) || !"0".equals(financeType)){
			financeType="1";//1:默认,投资端
		}
		flash.put("name", name);
		flash.put("code", code);
		...
		if(RegexUtils.isMobileNum(name)){
			//不为空，新版借款app登录
			if("0".equals(financeType)) {
				user.findUserByMobile(name,FinanceTypeEnum.BORROW.getCode());
			}else {//为空，旧版投资app登录
				user.findUserByMobile(name,FinanceTypeEnum.INVEST.getCode());
			}
		}else{
			flash.error("请使用手机号登录");
			goToLogin(financeType);
		}
	...
	private static void goToLogin(String financeType){
		if("1".equals(financeType)){//投资
			login();
		}else if("0".equals(financeType)){//借款
			loginLoan();
		}
	}
	/**
	...
	private static void goToRegister(String financeType){
		if("1".equals(financeType)){//投资
			register("","");
		}else if("0".equals(financeType)){//借款
			registerLoan("","");
     ...
     *un 推荐用户名称加密串
	 */
	public static void registerLoan(String un, String recommendUserName) {
		String loginOrRegister = Constants.LOGIN_AREAL_FLAG;
	...
	public static void registering() {
		...
		if (isMobileRegister) {
			...
			if(Constants.CHECK_MSG_CODE) {
				...
				if(cCode == null) {
					...
					if(StringUtils.isNotBlank(h5)) {
						renderText("短信校验码已失效，请重新点击发送校验码");
					}else {
						goToRegister(financeType);
					}
				}
			...
			User.isMobileExist(mobile, null,Integer.parseInt(financeType), error);
		...
		if (!RegexUtils.isValidPassword2(password)) {
			flash.error("请填写符合要求的密码");
			if(StringUtils.isNotBlank(h5)) {
				renderText("请填写符合要求的密码");
			}else {
				goToRegister(financeType);
			}
		}
		...
		}else if(StringUtils.isBlank(financeType)||"1".equals(financeType)){
...
//		}
	...
	public static void appRegNew(String recommendName) {
		String param = params.get("theme");
		if("20171108".equals(param)){
			render("app/views/front/account/LoginAndRegisterAction/appNewReg.html",recommendName);
		}
		render(recommendName);
	}
	...
	public static String appRegMobile(String name,String mobile,String smsCode,String password,String recommendName) throws IOException {
		...
		User.isMobileExist(mobile, null,financeType, error);
    ...
    public static void verifyMobileForReg(String mobile, String code, String randomID) {
		...
		if (!StringUtils.isBlank(financeType)&&"0".equals(financeType)) {
			if (User.isMobileExist(mobile, null, FinanceTypeEnum.BORROW.getCode(), error) < 0) {
				renderJSON(error);
			}
		} else if(StringUtils.isBlank(financeType)||"1".equals(financeType)){
			if (User.isMobileExist(mobile, null, FinanceTypeEnum.INVEST.getCode(), error) < 0) {
				renderJSON(error);
			}
		}
	...
	/**
	 * 
	 * @author xinsw 带短信验证码登录
	 * @creationDate 2018年3月12日
	 * @description
	 */
	public static void loginingCode() {
		//String encryString = Session.current().getId();
		//Integer loginCount = (Integer )Cache.get("loginCount" + encryString);
		//loginCount = loginCount == null ? 0 : loginCount;
		//Cache.set("loginCount"+encryString, ++loginCount, Constants.CACHE_TIME_MINUS_30);
		...
		Cache.delete("mobile_" + user.mobile);
		AccountHome.home();
	}
	
	public static void verifyMobileForLogin(String mobile, String code, String randomID) {
        ErrorInfo error = new ErrorInfo();
    	 ...
    	 */        
        if(Constants.CHECK_MSG_CODE){
        	Object cache = Cache.get("mobile_" + mobile);
        	if(null == cache){
        		Cache.set("mobile_" + mobile, mobile,"1min");
        	}else{
        		String isOldMobile = cache.toString();
        		if (isOldMobile.equals(mobile)) {
        			error.code = -1;
        ...
        if(User.isMobileExist(mobile, null, error) != -2){
        	error.msg = "手机号不存在";
        	error.code = -1;
			renderJSON(error);
		}

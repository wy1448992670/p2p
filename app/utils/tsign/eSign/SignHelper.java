...
public class SignHelper {
	...
	public static void initProject() {
		...
		// 项目ID(应用ID)
		proCfg.setProjectId(SceneConfig.PROJECT_ID);//
		// 项目Secret(应用Secret)
		proCfg.setProjectSecret(SceneConfig.PROJECT_SECRET); //
		// 开放平台地址
		// proCfg.setItsmApiUrl("http://121.40.164.61:8080/tgmonitor/rest/app!getAPIInfo2");//测试地址
	...
	public static FileDigestSignResult userPersonSignByStream(byte[] pdfFileStream, String accountId, String sealData) {
		...
		// 设置坐标定位签署的PosBean，坐标定位方式支持单页签章、多页签章和骑缝章，但对关键字签章指定页码无效；
		PosBean posBean = setKeyPosBean("", 40, 10, 90);
		// 设置签署类型为 关键字签章
	...
	public static FileDigestSignResult userPersonSignByStream(byte[] pdfFileStream, String accountId, String sealData,
		...
		PosBean posBean = setKeyPosBean(key, weight / 2 + 95, 5, weight);
		// 设置签署类型为 关键字签章
		SignType signType = SignType.Key;
	...
	public static FileDigestSignResult userPersonSignByFile(String srcPdf, String signedPdf, String accountId,
		...
		// 设置坐标定位签署的PosBean，坐标定位方式支持单页签章、多页签章和骑缝章，但对关键字签章指定页码无效；
		PosBean posBean = setKeyPosBean("甲方（借款人）", 0, 0, 159);
		// 设置签署类型为 关键字签章
		SignType signType = SignType.Key;
	...
	public static String addPersonAccount() {
		...
		// 姓名
		personBean.setName("龚宁媚 ");
		// 身份证号/护照号
		personBean.setIdNo("352201198712160025");
	...
	public static String addPersonAccount(String name, String idNo, int user_type) {
		PersonBean personBean = new PersonBean();
		OrganizeBean organizeBean = new OrganizeBean();
		AddAccountResult addAccountResult = new AddAccountResult();
		// 如果是企业 或者个体工商户
		if (UserTypeEnum.COMPANY.getCode() == user_type || UserTypeEnum.INDIVIDUAL.getCode() == user_type) {
			...
			organizeBean.setOrganCode(idNo);
			System.out.println("----开始创建企业账户...");
			AccountService accountService = AccountServiceFactory.instance();
			addAccountResult = accountService.addAccount(organizeBean);
			...
			System.out.println("----开始创建个人账户...");
			AccountService accountService = AccountServiceFactory.instance();
			addAccountResult = accountService.addAccount(personBean);
			if (0 != addAccountResult.getErrCode()) {
				LOG.info("创建个人账户失败，errCode=" + addAccountResult.getErrCode() + " msg=" + addAccountResult.getMsg());
				return null;
			} else {
	...
	public static String getAccountInfoByIdNo(String no,int user_type){
		AccountService accountService = AccountServiceFactory.instance();
		if(user_type==1) {
			GetAccountProfileResult result = accountService.getAccountInfoByIdNo(no, 11);
			if(result.getAccountInfo()==null) {
				return  "";
			}
			return  result.getAccountInfo().getAccountUid();
		}
		if(user_type==2||user_type==3) {
			GetAccountProfileResult result = accountService.getAccountInfoByIdNo(no, 22);
			if(result.getAccountInfo()==null) {
				return  "";
			}
			return  result.getAccountInfo().getAccountUid();
